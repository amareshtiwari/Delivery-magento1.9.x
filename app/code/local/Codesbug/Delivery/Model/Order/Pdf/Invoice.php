...
class Codesbug_Delivery_Model_Order_Pdf_Invoice extends Mage_Sales_Model_Order_Pdf_Invoice
{
...
//delivery date information
            ...
            $deliverydate = $order->getCodesbugDeliveryDate();
            $deliverytime = $order->getCodesbugDeliveryTimefirst() . '-' . $order->getCodesbugDeliveryTimelast();
            $deliverycomment = $order->getCodesbugDeliveryComment();
            if (!empty($deliverydate)) {
                $page->drawText('Delivery Date:' . $deliverydate, 285, $yShipments - $topMargin, 'UTF-8');
                $yShipments -= $topMargin + 10;

            }
            if (!empty($deliverytime)) {
                $page->drawText('Delivery Time:' . $deliverytime, 285, $yShipments - $topMargin, 'UTF-8');
                $yShipments -= $topMargin + 10;
            }

            $comm = Mage::getStoreConfig('datesection/date/displaycommentinpdf');
            if ($comm == 1) {
                if (!empty($deliverycomment)) {
                    //$deliverycomment = preg_replace('/<br[^>]*>/i', "\n", $deliverycomment);
                    //$page->drawText(strip_tags(ltrim($textLine)), 75, $line, 'UTF-8');
                    $page->drawText('Delivery Comment:' . strip_tags(ltrim($deliverycomment)), 285, $yShipments - $topMargin, 'UTF-8');
                    $yShipments -= $topMargin + 10;
